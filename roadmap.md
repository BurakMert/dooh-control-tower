...
## Today's start
**Next chunk: M1.6** — H3 indexing job: precompute res-8 and res-9 cells per screen, backfill the `h3_r8`/`h3_r9` columns we already added to `pop_event` and `screen`. Decisions to land at the top: (1) **H3 library** — Python's `h3` 4.x (compute Python-side, write via SQL) vs Postgres-side `h3-pg` extension (compute via `h3_lat_lng_to_cell()` SQL). Python is simpler to set up; h3-pg keeps compute in the DB and would let M1.6 be pure SQL migrations. (2) **Resolution rationale** — confirm r8 (~150m edge) for screen-cluster grouping and r9 (~55m) for single-screen-level. Both stored, M4 picks one per zoom level. (3) **Backfill strategy** — one-shot Python job vs Alembic data migration vs server-side trigger. The "compute once, then trigger" pattern fits append-only pop_event nicely. (4) **Index strategy** — add `(event_date, h3_r8)` btree (heatmap hot path), and a per-screen lookup index. Where do we put them — on the partitioned parent (auto-cascades) or per-partition? (5) **Screen-side h3 column** — `screen` doesn't have h3 yet; add it now (ALTER TABLE on a small table is cheap) so the live ad-request path (M3.4) can compute pop_event's h3 from `screen.h3_r8` instead of from `screen.geom` each time. Reflection-eligible (first compute-once index, first extension-vs-Python crossroads).
