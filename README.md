...
- [`m1-6-h3-indexing.html`](./docs/reflections/m1-6-h3-indexing.html) — Python `h3` 4.x over the h3-pg extension (DB-stays-a-DB principle, ADR-0002), no h3 on `screen`, no speculative Postgres btree indexes, "H3 indexing" vs "Postgres indexing" disambiguation, GROUP BY planner notes (M1.6)
